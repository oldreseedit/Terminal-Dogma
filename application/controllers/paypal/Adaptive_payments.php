...
class Adaptive_payments extends CI_Controller 
{
	...
	function Pay()
	{
		...
		$PayRequestFields = array(
								...
								'CancelURL' => 'https://www.reseed.it/cancel_payment', 									// Required.  The URL to which the sender's browser is redirected if the sender cancels the approval for the payment after logging in to paypal.com.  1024 char max.
								...
								'ReturnURL' => 'https://www.reseed.it/payment_accepted', 									// Required.  The URL to which the sener's browser is redirected after approvaing a payment on paypal.com.  1024 char max.
		...
		$Receivers = array();
		$Receiver = array(
						'Amount' => '0.01', 											// Required.  Amount to be paid to the receiver.
						'Email' => 'info@reseed.it', 												// Receiver's email address. 127 char max.
		...
		$AccountIdentifierFields = array(
										'Email' => 'tiziano.flati@gmail.com', 								// Sender's email address.  127 char max.
...
// 		print_r($PayPalResult);
		...
		{
			print("ERRORE DURANTE LA CHIAMATA ALLE API DI PAYPAL.");
...
// 				echo '<p><a href="' . $PayPalResult['RedirectURL'] . '">PROCEED TO PAYPAL</a></p>';
	...
	function Pay_with_options()
	{
		...
		print("<br/>ITEMS:"); print_r($cartItems);
		print("<br/>OPTIONS:"); print_r($cartOptions);
		...
		print("<br/>USER'S COURSES"); print_r($courses);
		...
		foreach($cartItems as $item)
		{
			...
			if(array_key_exists($courseID, $courses)) continue;
			...
			$totalItems[] = array('item' => $courseID, 'price' => $coursePrice);
			$total += $coursePrice;
			...
			if($wantSimulation)
			{
				...
				if($simulationPrice)
				{
					$totalItems[] = array('item' => $courseID . "-simulation", 'price' => $simulationPrice);
		...
		$seedOnDiscount = 0;
		// Prendi l'eventuale seedon che l'utente ha specificato e controlla che ce l'abbia davvero
		if($cartOptions['seedOnChosen'] && $cartOptions['seedOnChosen'] !== "-1")
		{
			...
			$userSeedons = [];
			foreach($this->seedon_model->get($userID) as $seedon)
			{
				$userSeedons[$seedon['seedonID']] = $seedon;
			}
			print("<br/>Seedon dell'utente: "); print_r($userSeedons);
			...
			if(array_key_exists($seedonChosen, $userSeedons))
			{
				...
				if($seedonInfo['tag'] === "DISCOUNT")
				{
					...
					$this->seedon_model->use_seedon($seedonChosen);
				}
			}
		}
		...
		{
			print("<br/>COSTO PRIMA:" . $item['price']);
			
			$item['price'] = $item['price'] - ($item['price'] * $totalDiscount);
			$totalItems[$key] = $item;
			
			print("<br/>COSTO DOPO:" . $item['price']);
		}
...
// 				'CountryCode' => 'IT', 				// Required.  2 char code of the home country of the end user.
// 				'DisplayName' => 'Tiziano Flati', 				// Required.  The full name of the consumer as known by the institution.  200 char max.
// 				'InstitutionCustomerEmail' => 'tiziano.flati', 	// The email address of the consumer.  127 char max.
// 				'FirstName' => 'Tiziano', 					// Required.  The first name of the consumer.  64 char max.
// 				'LastName' => 'Flati', 					// Required.  The last name of the consumer.  64 char max.
// 				'InstitutionCustomerID' => '1', 		// Required.  The unique ID assigned to the consumer by the institution.  64 char max.
// 				'InstitutionID' => 'Titto'				// Required.  The unique ID assiend to the institution.  64 char max.
		);
		...
		print("<br/>DETAILS: "); print_r($totalItems);
		
		foreach($totalItems as $item)
		{
			$InvoiceItem = array(
					'Name' => $item['item'], 								// Name of item.
					'Identifier' => $item['item'], 						// External reference to item or item ID.
					'Price' => $item['price'], 								// Total of line item.
					'ItemPrice' => $item['price'],							// Price of an individual item.
					'ItemCount' => '1'							// Item QTY
			);
			array_push($InvoiceItems,$InvoiceItem);
		}
		
		$InvoiceData['Item'] = $InvoiceItems;
...
/* Location: ./system/application/controllers/paypal/templates/Adaptive_payments.php */